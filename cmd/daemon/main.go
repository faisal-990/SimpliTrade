...
func main() {
	...
	rawStocks, err := os.ReadFile(dir + "/internal/engine/seed/mock_stocks_list.json")

	if err != nil {
		log.Fatal("Failed to open mock_stocks_list.json file")
	}
	var data []Stock
	json.Unmarshal(rawStocks, &data)

	// Load YAML strategy
	rawRules, err := os.ReadFile(dir + "/internal/engine/strategies/benjamin.yml")
	if err != nil {
		log.Fatal("Failed to open benjamin.yml file")
	}
